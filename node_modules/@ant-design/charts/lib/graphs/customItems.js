...
exports.registerOrganizationCardNode = exports.registerIndicatorCardNode = void 0;
...
var registerIndicatorCardNode = function () {
    g6_1.default.registerNode('indicator-card', {
        ...
        draw: function (cfg, group) {
            ...
            var _f = itemsCfg !== null && itemsCfg !== void 0 ? itemsCfg : {}, itemStyle = _f.style, itemContainerStyle = _f.containerStyle, layout = _f.layout, sort = _f.sort, _g = _f.padding, itemPadding = _g === void 0 ? [6, 0, 0] : _g;
            var itemPaddingArray = utils_1.getCssPadding(itemPadding);
            var _h = value, title = _h.title, items = _h.items;
            ...
            if (title) {
                ...
                titleShape = group.addShape('rect', {
                    attrs: __assign(__assign({ x: 0, y: 0, width: size[0], height: 0 }, constants_1.defaultTitleRectStyle), utils_1.getStyle(titleContainerStyle, cfg, group)),
                    name: 'title-rect',
                    draggable: true,
                });
                titleTextShape = group.addShape('text', {
                    attrs: __assign(__assign({ x: paddingArray[3], y: paddingArray[0], textBaseline: 'top', text: title }, constants_1.defaultTitleLabelStyle), utils_1.getStyle(titleStyle, cfg, group)),
                    name: 'title',
                });
                ...
                var createRowItems_1 = function (item, contentWidth, startX, index) {
                    ...
                    keys.forEach(function (key, keyIndex) {
                        ...
                        var keyShape = group.addShape(isIcon ? 'image' : 'text', {
                            attrs: __assign(__assign({ textBaseline: 'top', x: startX + x, y: height, text: item[key], img: item[key] }, (isIcon ? constants_1.defaultIconStyle : constants_1.defaultLabelStyle)), utils_1.getStyle(itemStyle || labelStyle, cfg, group, key)),
            ...
            // collapse marker
            if (markerCfg) {
                var _j = shape.getBBox(), shapeWidth_1 = _j.width, shapeHeight = _j.height;
                var _k = typeof markerCfg === 'function' ? markerCfg(cfg, group) : markerCfg, show = _k.show, _l = _k.position, position = _l === void 0 ? 'right' : _l, collapsed = _k.collapsed, markerStyle = _k.style;
                elements_1.createMarker({
                    show: show,
                    position: position,
                    collapsed: collapsed,
                    style: markerStyle,
        ...
        draw: function (cfg, group) {
            ...
            if (value) {
                ...
                var createRowItems_2 = function (item, contentWidth, startX, index) {
                    ...
                    keys.forEach(function (key, keyIndex) {
                        var isIcon = key.startsWith('icon');
                        var shapeStyle = utils_1.getStyle(labelStyle, cfg, group, key);
                        if (key === 'icon' && item[key]) {
                            iconWidth = shapeStyle.width || 32;
                        }
                        var keyShape = group.addShape(isIcon ? 'image' : 'text', {
                            attrs: __assign(__assign(__assign(__assign({ textBaseline: 'top', textAlign: 'center' }, getXY(key, shapeStyle)), { text: item[key], img: item[key] }), (isIcon ? constants_1.defaultIconStyle : constants_1.defaultLabelStyle)), shapeStyle),
                            name: key + "-" + index + "-" + keyIndex,
                        });
                        rowHeight.push(keyShape.getBBox().height);
                    });
